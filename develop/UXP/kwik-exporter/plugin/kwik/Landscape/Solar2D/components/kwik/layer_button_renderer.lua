local _M = require("components.kwik.layer_button").new(scene)
--
...
--
      ...
      model.x, model.y = app.ultimatePosition(Props.x, Props.y, Prosp.align)
...
function _M:localVars(UI)
    ...
    if model.bookTmplt then
        model.x, model.y, model.width, model.height, model.image = app.getModel(model.name, model.image, UI.dummy)
    end
...
function _M:didShow(UI)
    ...
    if model.btaps and layer[model.name] then
        if model.mask then
            local mask = graphics.newMask(app.imgDir .. model.mask, app.systemDir)
            layer[model.name]:setMask(mask)
        end
        self:addEventListenerForTap(
            UI,
            {
...
function _M:toDispose(UI)
    ...
    if model.btaps and layer[model.name] then
        self:removeEventListenerForTap(
            UI,
...
function _M:createButton(UI, model)
    ...
    -- Tap
    if model.btaps then
        obj = display.newImageRect(app.imgDir .. model.image, app.systemDir, imageWidth, imageHeight)
    else -- Press
        ...
        obj =
            widget.newButton {
            id          = model.name,
            defaultFile = app.imgDir .. imagePath,
            overFile    = app.imgDir .. model.over,
            width       = imageWidth,
            height      = imageHeight,
            onRelease   = onReleaseHandler,
            baseDir     = app.systemDir
        }
        --
