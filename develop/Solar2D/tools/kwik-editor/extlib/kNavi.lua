...
--local app.kNavig = nil
--local app.kNavigation = nil
...
Navigation.new = function( obj, params, naviListener)
	...
	app.kNavig = display.newGroup()
	--app.kNavig.anchorChildren = true
	--app.kNavig.anchorX = 0; app.kNavig.anchorY = 0;

	--app.kNavig.x = 0; app.kNavig.y = 0
	...
	local naviBackground = display.newRect( nLeft1, nTop1, nWidth1, nHeight1 )
       ...
       app.kNavig:insert(naviBackground);
    ...
    for i=1, table.maxn( pageList ) do
    	-- print(pageList[i][1])
			local path = system.pathForFile( pageList[i][1],  app.systemDir )
			if path then
	 		   menuTab[i] = display.newImage( pageList[i][1], app.systemDir)
			else
	   ...
	   app.kNavigation = widget.newScrollView {
          top = nTop,
          left = nLeft,
          width = nWidth,
          height = nHeight,
       ...
       --app.kNavigation.verticalScrollDisabled = nVert;
       --app.kNavigation.horizontalScrollDisabled = nHor;


       app.kNavigation:insert(navItems)
       app.kNavig:insert(app.kNavigation)

    if (params.dire == "top") then
        app.kNavig:translate(0, (display.contentHeight - display.safeActualContentHeight)/2)
    elseif (params.dire == "bottom") then
        app.kNavig:translate(0, (display.contentHeight - display.safeActualContentHeight)/2)
    elseif (params.dire == "left") then
        app.kNavig:translate((display.contentWidth - display.safeActualContentWidth)/2, 0)
    elseif (params.dire == "right") then
        app.kNavig:translate((display.contentWidth - display.safeActualContentWidth)/2, 0)
    end
     ...
     app.kNavig:addEventListener( "touch", touchNavig )
	...
	Navigation.show = function( obj, params )
	    ...
	    if (dire == "bottom") then
			...
			app.kNavigation:scrollToPosition{ x = tGo*-1, y = nil } --, time = 500, onComplete = scrollCompleted}
		...
		elseif (dire == "left") then
			...
			app.kNavigation:scrollToPosition{ x = nil, y = tGo*-1 }
		...
		local function hideAgain()
			if app.kNavig.alpha == 1 then
				Navigation.hide()
			end
		end
		app.timerStash.timer_nav = timer.performWithDelay( 5000, hideAgain )
