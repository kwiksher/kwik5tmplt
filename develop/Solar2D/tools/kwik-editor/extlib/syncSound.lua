...
--print("step 2 - write words ")
	...
	local dir = app.audioDir .. sentenceDir
...
--[[ REVIEW THIS WHEN ENABLE THE PLAY AUDIO PER WORD --]]
	  ...
	  if wordTouch then
		...
		if (name=="" or name==nil) then
		  ...
		  if (lang=="") then
		  	words[i].snd = audio.loadSound(dir..name ..".mp3", app.systemDir)
		  else
			  	words[i].snd = audio.loadSound(dir..lang.."_"..name ..".mp3", app.systemDir)
		  end
		  words[i].id = i
...
function saySentence( params )
		...
		if (app.kwk_readMe==0) then return end
		...
		local syncClosure = function()
				...
				for i = 1,#line do
						...
						if text[i].trigger ~= nil then
								app.timerStash['syncSoundTrigger'..i] = timer.performWithDelay( line[i].start, text[i].trigger)
								table.insert(activeRead2me.syncSoundTrigger, app.timerStash['syncSoundTrigger'..i])
						end
				end
		end --syncClosure
		--
		app.timerStash.syncTimer = timer.performWithDelay(_wait, syncClosure)
end --saySentence
...
function speakWord( event )
	local word = event.target
 	...
 	local ePad   = app.isUltimateConfig and 10/4 or 10
...
--]]
    ...
    ----3.6 editing
	app.timerStash.timer_ss1 = timer.performWithDelay( transTime * 1000, function()
