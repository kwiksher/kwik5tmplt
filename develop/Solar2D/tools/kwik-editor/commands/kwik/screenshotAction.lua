...
function _M:takeScreenShot(layer, ptit, pmsg, shutter, buttonArr)
    app.screenshot:takeScreenShot(layer, ptit, pmsg, shutter, buttonArr)
end
--

app.asmModel = {
  sandboxPath = system.pathForFile( nil, app.DocumentsDir ),
  projPath    = app.kwikDir,
  APNG        = {},
  AGIF        = {}
}
--
  ...
  if system.getInfo("environment") == "simulator" then
    ...
    function APNGGen()
      ...
      if index == numFrames then
            table.insert(app.asmModel.APNG, {
                fileName    = fileName,
                fps         = fps,
                delay       = delayGM,
                loop        = loop
            ...
            --
            table.insert(app.asmModel.AGIF, {
                fileName    = fileName,
                fps         = fps,
                delay       = delayGM,
                loop        = loop
            ...
            if not file then
                ...
                output = lustache:render(contents, app.asmModel)

                local path = app.kwikDir.."/../assemble."..ext
    ...
    app.timerStash.scrnRecord = timer.performWithDelay( delay, APNGGen , numFrames )
  end
end
--
return _M
