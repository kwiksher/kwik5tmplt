...
function _M:recordAudio(duration, mmFile, malfa, sceneGroup, allAudios)
  ...
  local filePath = system.pathForFile(mmFile, app.DocumentsDir )
  ...
  local audioClosure = function(event )
    ...
    if file then
       io.close(file)
       allAudios.playback = audio.loadStream( mmFile, app.DocumentsDir )
       audio.play(allAudios.playback, {channel=31})
    end
   end
   app.timerStash.recTimer = timer.performWithDelay(duration, audioClosure )
   -- audio.stop(31)
...
function _M:playAudio(vaudio, vchan, vrepeat, vdelay, vloop, toFade, vvol, tm, trigger, params)
  ...
  if vdelay == 0 then
    ...
    app.timerStash[tm] = timer.performWithDelay(vdelay,
      myClosure, 1)
  end
end
--
