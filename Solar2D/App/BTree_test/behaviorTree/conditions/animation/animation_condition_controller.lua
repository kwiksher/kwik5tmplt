...
-- Module paths
local modulePaths = {
    animation = "conditions.animation.animation_conditions",
}
...
function M.initialize(objects)
    ...
    for name, path in pairs(modulePaths) do
        ...
        if success then
            ...
            print("Animation Condition Controller: Loaded module '" .. name .. "' from " .. path)
        else
            print("Warning: Failed to load condition module '" .. name .. "': " .. tostring(module))
        end
    end

    print("Animation Condition Controller: Initialized successfully")
end
