...
-- Supports multiple choice types (calm, fight, retreat, reload, continue, etc.)
...
function M.evaluate(conditionType)
    print(M.CONDITION_NAME .. " condition: checking " .. tostring(conditionType) .. " against playerChoice=" .. tostring(sceneObjects.playerChoice))
...
function M.generateConditions()
    ...
    for _, conditionType in pairs(M.CONDITION_TYPES) do
        ...
        -- Create local copy to avoid closure issues
        local localConditionType = conditionType
        conditionWrappers[conditionName] = {
            initialize = M.initialize,
            evaluate = function()
                return M.evaluate(localConditionType)
            end
        }
    end
    ...
    instance.generateConditions = function()
        ...
        for _, conditionType in pairs(instance.CONDITION_TYPES) do
            ...
            conditionWrappers[fullConditionName] = {
                ...
                evaluate = function()
                    return instance.evaluate(localConditionType)
                end
            }
        end
