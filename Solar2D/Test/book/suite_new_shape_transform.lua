...
function M.xtest_new_rectangle()
  M.UI.testCallback = function()
    ...
    local props = shape.drawRect(M.UI, function(phase, props)
      print(M.json.prettify( props ))
    end)
...
function M.xtest_new_text()
  M.UI.testCallback = function()
    ...
    -- then drag to a new position
    local shape = require("editor.shape.index")
    local props = shape.drawText(M.UI, function(phase, v)
      if phase == "ended" then
        print(M.json.prettify( v ))
      elseif phase == "editor" then
          local obj = v.field
          obj:dispatchEvent{name="userInput", phase = "editor", target=obj, newCharacters="hello"}
...
function M.xtest_move()
   M.UI.testCallback = function()
    ...
    --
    local name = "title"
    local target = M.util.getLayer(M.UI, name)
    print(target.text)
    --
    ...
    -- then drag to a new position
    local shape = require("editor.shape.index")
    shape.move(M.UI, target, function(phase, props)
...
function M.xtest_scale()
  M.UI.testCallback = function()
   ...
   -- then drag to a new position
   local shape = require("editor.shape.index")
   shape.scale(M.UI, target, function(props)
      print(M.json.prettify( props ))
   end)
...
function M.xtest_rotate()
  local obj = helper.selectBook("bookFree")
   ...
   M.UI.testCallback = function()
    ...
    -- then drag to a new position
    local shape = require("editor.shape.index")
    shape.rotate(M.UI, target, function(props)
      print(M.json.prettify( props ))
    end)
