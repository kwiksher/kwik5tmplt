...
function M.xtest_create_praticles()
  M.selectors.componentSelector.iconHander()
  M.selectors.assetsSelector:iconHander()
  M.selectors.assetsSelector:onClick(true, "particles")
  --select firstone
  helper.clickAsset(M.assetTable.objs, "particles/particle.png")
end

function M.test_create_sprite()
  M.selectors.componentSelector.iconHander()
  M.selectors.assetsSelector:iconHander()
  M.selectors.assetsSelector:onClick(true, "sprites")
  helper.clickAsset(M.assetTable.objs, "sprites/sprites.png")
end
...
function M.xtest_lfs()
  ...
  assert_not_nil(M.UI.editor.assets.videos)
  assert_equal(M.UI.editor.assets.videos[1].name, expected.videos[1].name)

  --select kwikplanet.mp4
  M.assetTable.objs[1]:touch({phase="ended"})

  -- audio
  assert_equal(M.UI.editor.assets.audios[1].name, expected.audios[1].name)
...
function M.xtest_save_video_renderAssets()
  ...
  local callScriptCommand = function(fileInSandbox, layer, filename)
    ...
    M.scriptsCommands.copyFiles(files, _TMP)
    --
  ...
  for i, entry in next,M.UI.editor.assets.videos do
    print(i, entry.name)
...
function M.xtest_save_audio()
  ...
  for i, entry in next,M.UI.editor.assets.audios do
    if entry.name == model.url then
        asset(entry.links[1].page == expected.audios[1].links[1].page)
        -- success --
        M.scriptsCommands.copyFiles({fileInSandbox}, _TMP)
      return
    end
  end
  assert(false)
