...
function M.xtest_new_page()
  ...
  book:touch()
  if M.UI.book ~="mybook" then
    return
  end
  for i, v in next, M.pageTable.iconObjs  do print(v.name  ) end
  local obj = M.pageTable.iconObjs[2]
...
function M.xtest_delete_page()
  ...
  for i, v in next, M.pageTable.objs  do print(v.page  ) end
  local obj = M.pageTable.objs[#M.pageTable.objs]
  printKeys(obj)
  obj:tap{phase = "ended"}

  local icon = M.pageTable.iconObjs[3]
