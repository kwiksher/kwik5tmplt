...
function M.xtest_wtildCardedString()
  ...
  print(M.wildcard.matchesWildCardedString(test, '?page1*')) -- false
  print(M.wildcard.matchesWildCardedString(test, '*page*'))
  print(M.wildcard.matchesWildCardedString(test, '*.png'))
  print(M.wildcard.matchesWildCardedString(test, '/page1/*.png'))
  print(M.wildcard.matchesWildCardedString(test, '/page?/tes?.png'))
  ...
  https://www.domoticz.com/forum/viewtopic.php?t=29900
  '*' M.wildcard for 0 or more chars.
  '?' M.wildcard for 1 char
  ```
  local function matchesWildCardedString(stringToMatch, wildCardedString)
    ...
    if wildCardedString:find('*') and wildCardedString:find('?') then
        dz.log('Not possible to combine single char M.wildcard(s) "?" with multi char M.wildcard(s) "*"',dz.LOG_ERROR)
        return false
