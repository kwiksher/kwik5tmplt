...
function Request:post()
  ...
  --
  --for k, v in pairs(self._headers) do print(k, v) end
  if self._headers["content-type"]:find("application") ~=nil then
    return data
  else
...
function Request:put()
  if self:method() ~= 'PUT' then return nil end
  self:headers()
  local data = self:receiveBody()
  --
  --for k, v in pairs(self._headers) do print(k, v) end
...
function Request:headers()
  ...
  if self._headerParsed then
    -- for k, v in paires(self._headers) do print(k, v) end
  ...
  --print("", self._contentLength)
  return self._headers
end

function Request:receiveBody(size)

  -- print("Request:reciveBody")
  size = size or self._contentLength

  -- print("", "self._contentLength", self._contentLength)
  -- print("", "self._contentDone", self._contentDone)
